...
def composer_code_parcelle(
    ...
    """
    Compose le code cadastral complet à partir de composants déjà séparés :
    code INSEE de la commune (5 chiffres), section (lettres) et numéro.

    Utilisé quand la commune, la section et le numéro proviennent de colonnes
    distinctes (ou ont déjà été extraits par le parseur de parcelles), plutôt
    que de normaliser une chaîne brute unique (voir `normaliser_parcelle`).
    """
    section = str(section or "")
    numero = str(numero or "")
    prefixe = str(prefixe or "")
    ...
    return f"{code_insee_commune}{prefixe}{section}{numero}"
